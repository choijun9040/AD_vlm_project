...
def build_scene_frame_order(drivelm_json_path: str):
    """
    DriveLM JSON에서 {scene_token: [frame_token, ...]} (시간순 정렬)과
    {frame_token: scene_token} 역매핑을 만든다.

    key_frames 딕셔너리 순서는 시간순이 아니므로, image_paths["CAM_FRONT"]
    파일명에 박혀 있는 마이크로초 유닉스 타임스탬프로 직접 정렬해야 한다.
    """
    with open(drivelm_json_path) as f:
        raw = json.load(f)
    ...
    for scene_token, scene_data in raw.items():
        ...
        for frame_token, frame_data in scene_data["key_frames"].items():
            cam_front_path = frame_data.get("image_paths", {}).get("CAM_FRONT", "")
            m = _CAM_FRONT_TS_PATTERN.search(cam_front_path)
            if m:
                ts = int(m.group(1))
            else:
                ts = 0
                ts_parse_failures += 1
            frame_ts.append((ts, frame_token))
            frame_to_scene[frame_token] = scene_token
        frame_ts.sort(key=lambda x: x[0])
        scene_frame_order[scene_token] = [ft for _, ft in frame_ts]

    if ts_parse_failures:
        print(f"[build_scene_frame_order] 경고: 타임스탬프 파싱 실패 {ts_parse_failures}건 (ts=0 폴백)")
    print(f"[build_scene_frame_order] {len(scene_frame_order)}개 scene 시간순 정렬 완료")

    return scene_frame_order, frame_to_scene


def _get_temporal_frame_tokens(frame_token, scene_frame_order, frame_to_scene, k):
    """현재 frame_token 기준 과거 k개(현재 포함) frame_token을 시간순으로 반환.

    scene 내 프레임이 k개 미만이면 가장 이른 프레임을 반복해서 채운다
    (자연스럽게 "시간적 신호 없음"으로 축소되므로 별도 분기 불필요).
    """
    scene_token = frame_to_scene.get(frame_token)
    if scene_token is None or scene_token not in scene_frame_order:
        return [frame_token] * k

    order = scene_frame_order[scene_token]
    try:
        idx = order.index(frame_token)
    except ValueError:
        return [frame_token] * k

    start = max(0, idx - k + 1)
    window = order[start: idx + 1]
    if len(window) < k:
        window = [window[0]] * (k - len(window)) + window
    return window
...
def _build_teacher_inputs(processor, frame_image_paths, question, answer, temporal_k, teacher_max_length):
    ...
    패턴이지만, 이미지가 K장이라 image_pad placeholder도 K개 남는다 — q_len 공식이
    "-1"이 아니라 "-temporal_k"가 되는 이유.
    """
    images = [Image.open(p).convert("RGB") for p in frame_image_paths]

    messages_q = [{
        "role": "user",
        "content": [{"type": "image", "image": p} for p in frame_image_paths]
                    + [{"type": "text", "text": question}],
    }]
    text_q = processor.apply_chat_template(
    ...
    q_text_ids = processor.tokenizer(
        text_q, return_tensors="pt", padding=False, add_special_tokens=True,
    ).input_ids
    ...
    return {
        ...
        "teacher_attention_mask": inputs["attention_mask"].squeeze(0),
        # 샘플당 K개 이미지라 squeeze(0) 하지 않음 — collate_fn에서 torch.cat으로 배치 결합
        "teacher_pixel_values":   inputs["pixel_values"],
        "teacher_image_grid_thw": inputs["image_grid_thw"],
        "teacher_q_len":          torch.tensor(q_len, dtype=torch.long),
    }
...
class DriveLMDataset(Dataset):
    ...
    def __init__(
        ...
        self.scene_frame_order = scene_frame_order or {}
        self.frame_to_scene = frame_to_scene or {}
        self.token_to_images = token_to_images or {}
    ...
    def __getitem__(self, idx):
        ...
        if self.processor is not None:
            ...
            item = {
                ...
                "labels":          labels,
                "q_len":           torch.tensor(q_len, dtype=torch.long),
            ...
            if self.temporal_k > 1:
                frame_seq = _get_temporal_frame_tokens(
                    s["frame_token"], self.scene_frame_order, self.frame_to_scene, self.temporal_k
                )
                frame_paths = [
                    str(self.token_to_images.get(ft, {}).get(self.use_camera, img_path))
                    for ft in frame_seq
                ]
...
class NuScenesQADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.processor is not None:
            ...
            if self.temporal_k > 1:
                frame_seq = _get_temporal_frame_tokens(
                    s["sample_token"], self.scene_frame_order, self.frame_to_scene, self.temporal_k
                )
                frame_paths = [
                    str(self.token_to_images.get(ft, {}).get(self.use_camera, img_path))
                    for ft in frame_seq
...
def create_unified_dataloader(
    ...
    # temporal_k > 1일 때만 scene/시간순 매핑 구축 (기존 호출부는 전혀 영향 없음)
    scene_frame_order, frame_to_scene = {}, {}
    if temporal_k > 1:
        scene_frame_order, frame_to_scene = build_scene_frame_order(drivelm_json)
    ...
    ds_drivelm = DriveLMDataset(
        ...
        scene_frame_order=scene_frame_order,
