...
| | 범위 | 소요 | 논문 기여 |
|---|---|---|---|
| **A. 최소 검증 (권장)** | **비전 타워만** TensorRT fp16으로 올려 붕괴 재현 여부 확인 | 반나절~하루 | **핵심 구멍을 메운다** |
| B. 전체 배포 | VLM 전체(INT4 LLM + 비전 타워) 추론 파이프라인 | 수일 | 부가적 |

**A로 충분하다.** 검증하려는 명제가 "fp16 연산 경로에서 비전 타워 마지막 블록이
표현 범위를 넘는가"이므로 LLM은 필요 없다. 전체 배포는 별개의 엔지니어링 과제다.

**메모리 타당성**: Qwen2.5-VL-3B 비전 타워는 depth 32 / hidden 1280 / intermediate 3420,
...
확인할 것:
- **TensorRT가 bf16을 지원하는가.** 지원하면 "비전 타워만 bf16 유지"라는 대응책이
...
### 2-1. 비전 타워를 ONNX로 내보낸다 (x86 개발 머신에서)

해상도를 **고정**한다. Qwen2.5-VL 비전 타워는 `grid_thw`로 회전 위치 임베딩과 윈도우
...
Jetson용 PyTorch로 비전 타워를 직접 fp16 실행한다. TensorRT의 융합 정책은 검증하지
...
- **지연시간·메모리** — 현재 논문에 경량화를 표방하면서 효율 수치가 하나도 없다.
  비전 타워만이라도 해상도별 지연시간을 재면 3장에 표 하나가 채워진다.
...
3. 비전 타워와 LLM의 **연산 dtype이 서로 다를 수 있음**에 주의.
   본 연구의 문제가 정확히 이 지점에서 발생했다(LLM은 INT4, 비전 타워는 저장 dtype)
